...
function exportFilterProvider.postProcessRenderedPhotos(functionContext, filterContext)
    -- NO GUESSING: We write a physical log file to the Desktop to track the exact point of failure.
    ...
    local status, err = pcall(function()
        ...
        for sourceRendition, renditionToSatisfy in filterContext:renditions{ plugin = _PLUGIN } do
            ...
            log("2. Render finished. Success: " .. tostring(success) .. " | Path: " .. tostring(pathOrMessage))
        ...
        if #imagePaths > 0 then
            ...
            -- Build JSON via concatenation to avoid string.format % crashes
            local payload = '{' ..
    ...
    if not status then
        log("FATAL CRASH: " .. tostring(err))
        LrDialogs.message("Lua Crash", tostring(err), "critical")
    end
